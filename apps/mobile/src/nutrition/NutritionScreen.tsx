...
function formatKcal(meal: NutritionMeal): string {
...
function sourceLabel(source?: string | null): string {
  ...
  return source || 'Source manquante';
}
...
function mealTypeLabel(mealType?: string | null): string | null {
  ...
  return MEAL_TYPE_OPTIONS.find((option) => option.key === mealType)?.label || mealType;
}

function formatDatasetVersions(datasetVersions?: Record<string, string> | null): string | null {
  const entries = Object.entries(datasetVersions ?? {});
  if (!entries.length) {
    return null;
  }
  return entries.map(([source, version]) => `${sourceLabel(source)} ${version}`).join(' · ');
}
...
function analysisJobLabel(status?: string | null): string {
  switch (status) {
    case 'pending':
...
function analysisJobText(meal: NutritionMeal): string | null {
  const job = meal.analysis_job;
  if (!job) {
    return null;
  }
  return `Job · ${analysisJobLabel(job.status)} · tentative ${job.attempts}`;
}

export default function App({ embedded = false, active = true }: { embedded?: boolean; active?: boolean }) {
  return (
    <SafeAreaProvider initialMetrics={initialWindowMetrics ?? FALLBACK_SAFE_AREA_METRICS}>
      <NutritionApp embedded={embedded} active={active} />
    </SafeAreaProvider>
  );
}

function NutritionApp({ embedded, active }: { embedded: boolean; active: boolean }) {
  ...
  const [status, setStatus] = useState('Initialisation Nutrition...');
  ...
  useEffect(() => {
    ...
    loadSettings()
      ...
      .catch((error) => setStatus(error instanceof Error ? error.message : 'Configuration indisponible'))
      .finally(() => {
  ...
  async function loadMealList(nextSettings = settings, quiet = false) {
    if (!quiet) {
      setStatus('Chargement du journal...');
    }
    try {
      ...
      if (quiet && currentSelected && mergedSelected && currentSelected.id === mergedSelected.id) {
        if (currentSelected.status === 'analyzing' && mergedSelected.status === 'ready') {
          setStatus('Analyse prête à vérifier');
        } else if (currentSelected.status === 'analyzing' && mergedSelected.status === 'needs_review') {
          setStatus('Analyse prête, correction requise');
        } else if (currentSelected.status === 'analyzing' && mergedSelected.status === 'error') {
          setStatus('Analyse en erreur, relance possible');
        }
      }
      if (readyIds.length > 0) {
        await notifyMealReady();
      }
      if (!quiet) {
        setStatus(`${result.meals.length} repas dans le journal`);
      }
    } catch (error) {
      if (!quiet) {
        setStatus(error instanceof Error ? error.message : 'Journal indisponible');
      }
      throw error;
    }
  }
  ...
  function addPickedPhotos(photos: LocalPhoto[], emptyMessage: string) {
    ...
    setStatus(`${photos.length} photo(s) ajoutée(s) au repas`);
    setScreen('add');
  }
  ...
  async function recoverPendingPickerResults() {
    try {
      ...
      if (recoveredPhotos.length > 0) {
        addPickedPhotos(recoveredPhotos, 'Aucune photo récupérée');
      }
      return recoveredPhotos.length;
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Récupération photo indisponible');
      return 0;
    }
  }
  ...
  async function takePhoto() {
    if (selectedPhotos.length >= MAX_PHOTOS_PER_MEAL) {
      setStatus('Limite de 8 photos atteinte');
      return;
    }
    try {
      const permission = await ImagePicker.requestCameraPermissionsAsync();
      if (permission.status !== 'granted') {
        Alert.alert('Caméra indisponible', "La permission caméra est nécessaire pour prendre une photo.");
        setStatus('Permission caméra refusée');
        return;
      }
      ...
      addPickedPhotos(photosFromPickerResult(result), 'Aucune photo prise');
    } catch (error) {
      const recoveredCount = await recoverPendingPickerResults();
      if (recoveredCount > 0) {
        return;
      }
      setStatus(error instanceof Error ? error.message : 'Caméra indisponible');
      Alert.alert('Caméra indisponible', error instanceof Error ? error.message : 'Impossible de lancer la caméra.');
    }
  }
  ...
  async function choosePhotos() {
    ...
    try {
      ...
      addPickedPhotos(photosFromPickerResult(result), 'Aucune photo sélectionnée');
    } catch (error) {
      const recoveredCount = await recoverPendingPickerResults();
      if (recoveredCount > 0) {
        return;
      }
      setStatus(error instanceof Error ? error.message : 'Galerie indisponible');
  ...
  async function uploadMeal() {
    if (!selectedPhotos.length) {
      Alert.alert('Ajoute au moins une photo', 'Photographie le repas, un emballage, une étiquette ou un code-barres visible.');
      return;
    }
    setUploading(true);
    setStatus('Envoi du repas...');
    try {
      ...
      setStatus('Analyse lancée en local');
    } catch (error) {
  ...
  async function openMeal(meal: NutritionMeal) {
    ...
    try {
      ...
      setStatus(error instanceof Error ? error.message : 'Repas indisponible');
    }
  }
  ...
  async function refreshSelectedMeal() {
    ...
    setStatus('Actualisation du repas...');
    try {
      ...
      if (result.meal.status === 'ready') {
        ...
        setStatus('Analyse toujours en cours');
      }
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Actualisation impossible');
    } finally {
  ...
  async function applyEdits() {
    ...
    try {
      ...
      setEdits([]);
      await loadMealList({ ...settings, deviceToken: result.token }, true);
      setStatus('Repas recalculé');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Correction impossible');
    } finally {
  ...
  async function validateMeal() {
    ...
    try {
      ...
      selectedMealRef.current = result.meal;
      await loadMealList({ ...settings, deviceToken: result.token }, true);
      setStatus('Repas validé dans ALIS');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Validation impossible');
    } finally {
      setSavingReview(false);
    }
  }
  ...
  async function reanalyzeMeal() {
    ...
    try {
      ...
      await loadLearnedReferencesForMeal(result.meal);
      await loadMealList({ ...settings, deviceToken: result.token }, true);
      setStatus('Analyse relancée en local');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "Relance de l'analyse impossible");
    } finally {
      setSavingReview(false);
    }
  }
  ...
  async function searchFoodReferences(itemId: string) {
    ...
    if (query.length < 2) {
      setStatus('Recherche trop courte');
      return;
    }
    ...
    try {
      ...
      setStatus(`${result.foods.length} référence(s) trouvée(s)`);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Recherche nutrition indisponible');
    } finally {
  ...
  async function matchFoodReference(itemId: string, reference: NutritionFoodReference) {
    ...
    try {
      ...
      await loadLearnedReferencesForMeal(result.meal);
      await loadMealList({ ...settings, deviceToken: result.token }, true);
      setStatus('Aliment associé à une source nutritionnelle');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Association impossible');
    } finally {
      setSavingReview(false);
    }
  }
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.tabs}>
        <Pressable style={[styles.tab, screen === 'journal' && styles.tabActive]} onPress={() => setScreen('journal')}>
          <Text style={[styles.tabText, screen === 'journal' && styles.tabTextActive]}>Journal</Text>
        </Pressable>
        <Pressable style={[styles.tab, screen === 'review' && styles.tabActive]} onPress={openReviewTab}>
          <Text style={[styles.tabText, screen === 'review' && styles.tabTextActive]}>Revue</Text>
        </Pressable>
      </View>
      ...
      {loading ? (
        ...
        <ReviewEmptyScreen onAdd={() => setScreen('add')} onJournal={() => setScreen('journal')} />
      ) : (
        <JournalScreen
          meals={sortedMeals}
...
function JournalScreen({
  ...
  const daySections = buildJournalDaySections(meals);
  return (
    <ScrollView
    ...
    >
      {meals.length === 0 ? (
        <View style={styles.emptyState}>
          <Text style={styles.emptyTitle}>Aucun repas analysé</Text>
          <Text style={styles.secondaryText}>Ajoute plusieurs photos d’un repas, d’un emballage ou d’une étiquette.</Text>
        </View>
      ) : (
          ...
          <View key={section.key} style={styles.daySection}>
            <View style={styles.dayHeader}>
              ...
              <Text style={styles.daySummary}>
                {section.validatedKcal} kcal validées · P {section.proteinG} g · G {section.carbohydratesG} g · L {section.fatG} g
              </Text>
              {section.pendingCount > 0 ? (
                <Text style={styles.pendingText}>{section.pendingCount} repas à traiter</Text>
              ) : (
                <Text style={styles.validatedText}>Tout est à jour</Text>
              )}
              {section.title === "Aujourd'hui" ? (
                <Text style={styles.alisSummary}>ALIS · {section.validatedKcal} kcal validées aujourd’hui</Text>
              ) : null}
            </View>
            {section.meals.map((meal) => {
              const jobText = analysisJobText(meal);
              ...
              return (
                <Pressable key={meal.id} style={styles.mealRow} onPress={() => onOpenMeal(meal)}>
                  <View style={styles.mealRowTop}>
                    <Text style={styles.mealTitle}>{meal.title || mealTypeLabel(meal.meal_type) || 'Repas'}</Text>
                    <Text style={[styles.badge, badgeStyle(meal.status)]}>{mealStatusLabel(meal.status)}</Text>
                  </View>
                  <Text style={styles.kcalText}>{formatKcal(meal)}</Text>
                  <Text style={styles.secondaryText}>
                    {meal.photo_count ?? 0} photo(s) · {confidenceInsight(meal)}
                  </Text>
                  <Text style={styles.journalAlisText}>{alisImpactLabel(meal)}</Text>
                  <Text style={styles.mealNextAction}>{mealNextActionLabel(meal)}</Text>
                  ...
                  {canQuickDelete ? (
                    ...
                    >
                      <Text style={styles.inlineDangerButtonText}>Supprimer</Text>
                    </Pressable>
                  ) : null}
                </Pressable>
              );
    ...
    <ScrollView style={styles.content}>
      ...
      <View style={styles.mealTypeGrid}>
        {MEAL_TYPE_OPTIONS.map((option) => {
          ...
          return (
            ...
            >
              <Text style={[styles.mealTypeText, selected && styles.mealTypeTextActive]}>{option.label}</Text>
            </Pressable>
          );
        })}
      </View>
      ...
      <View style={styles.actionGrid}>
        <Pressable style={styles.actionButton} onPress={onTakePhoto}>
          ...
          <Text style={styles.actionHint}>Ajoute le repas, puis reviens prendre l’emballage, l’étiquette ou le code-barres.</Text>
        </Pressable>
        <Pressable style={styles.actionButton} onPress={onChoosePhotos}>
          <Text style={styles.actionTitle}>Choisir des photos</Text>
          <Text style={styles.actionHint}>Sélection multiple depuis la galerie.</Text>
        </Pressable>
      </View>

      <View style={styles.aiHintPanel}>
        <Text style={styles.sectionTitle}>Indications pour l'IA</Text>
        <TextInput
          style={styles.notesInput}
          value={notes}
          onChangeText={onNotesChange}
          placeholder="Ex. riz, poulet, sauce creme, portion partagee..."
          multiline
        ...
        <TextInput
          ...
          autoCorrect={false}
        />
        <Text style={styles.secondaryText}>Utile si l'emballage n'est pas lisible ou si tu veux préciser un aliment.</Text>
      </View>

      <Text style={styles.sectionTitle}>
        {photos.length}/{MAX_PHOTOS_PER_MEAL} photo(s) sélectionnée(s)
      </Text>
      {photos.length === 0 ? (
        <Text style={styles.addHintText}>Ajoute au moins une photo pour lancer l’analyse.</Text>
      ) : (
        <View style={styles.addReadyPanel}>
          <Text style={styles.addReadyTitle}>Photos prêtes pour l'analyse locale</Text>
          <Text style={styles.secondaryText}>Ajoute le repas, un emballage, une étiquette ou un code-barres visible.</Text>
        </View>
      )}
      ...
      <Pressable style={[styles.primaryButton, !canUpload && styles.disabledButton]} disabled={!canUpload} onPress={onUpload}>
        <Text style={styles.primaryButtonText}>{uploading ? 'Envoi...' : "Lancer l'analyse"}</Text>
      </Pressable>
    </ScrollView>
  );
}

function ReviewEmptyScreen({ onAdd, onJournal }: { onAdd: () => void; onJournal: () => void }) {
  return (
    <View style={[styles.content, styles.emptyReviewContent]}>
      <View style={styles.emptyState}>
        <Text style={styles.emptyTitle}>Aucun repas à revoir</Text>
        <Text style={styles.secondaryText}>Ajoute d’abord des photos ou ouvre un repas existant depuis le journal.</Text>
      </View>
      <Pressable style={styles.primaryButton} onPress={onAdd}>
        <Text style={styles.primaryButtonText}>Ajouter un repas</Text>
      </Pressable>
      <Pressable style={styles.secondaryButton} onPress={onJournal}>
        <Text style={styles.secondaryButtonText}>Voir le journal</Text>
      </Pressable>
    </View>
  );
}
...
function ReviewScreen({
  ...
  const progress = analysisProgress(meal);
  const reviewMealTitle = meal.title || mealTypeLabel(meal.meal_type) || 'Repas';
  return (
    <ScrollView style={styles.content} keyboardShouldPersistTaps="handled">
      <Pressable onPress={onBack}>
        <Text style={styles.linkText}>Retour au journal</Text>
      </Pressable>
      <View style={styles.summaryBand}>
        <Text style={styles.sectionTitle}>Étude du repas</Text>
        <Text style={styles.summaryMealType}>{reviewMealTitle}</Text>
        <Text style={styles.summaryKcal}>{formatKcal(meal)}</Text>
        <Text style={styles.secondaryText}>
          {mealStatusLabel(meal.status)} · {confidenceInsight(meal)}
        </Text>
        <Text style={styles.macroText}>{formatMacros(meal)}</Text>
        <Text style={styles.impactText}>{alisImpactLabel(meal)}</Text>
        <AnalysisTrace meal={meal} />
        {meal.error_message ? <Text style={styles.errorText}>{meal.error_message}</Text> : null}
        <AnalysisJobPanel meal={meal} />
      </View>

      <ReviewDiagnosticPanel meal={meal} />
      ...
      {isAnalyzing ? (
        <View style={styles.statePanel}>
          ...
          <Text style={styles.secondaryText}>
            Étape {progress.currentStep}/{progress.totalSteps} · {progress.detail}
          </Text>
          <Text style={styles.secondaryText}>Tu peux revenir au journal, Nutrition se mettra à jour automatiquement.</Text>
          <Pressable style={[styles.secondaryButton, refreshing && styles.disabledButton]} disabled={refreshing} onPress={onRefresh}>
            <Text style={styles.secondaryButtonText}>{refreshing ? 'Actualisation...' : 'Actualiser maintenant'}</Text>
          </Pressable>
        </View>
      ) : null}
      ...
      {isError ? (
        <View style={styles.statePanel}>
          <Text style={styles.stateTitle}>Analyse à relancer</Text>
          <Text style={styles.secondaryText}>L’IA locale n’a pas rendu une étude exploitable pour ce repas.</Text>
          <Pressable style={[styles.secondaryButton, saving && styles.disabledButton]} disabled={saving} onPress={onReanalyze}>
            <Text style={styles.secondaryButtonText}>Relancer l'analyse</Text>
          </Pressable>
        </View>
      ) : null}
      ...
      {canDelete && (isAnalyzing || isError) ? (
        <Pressable style={[styles.dangerButton, saving && styles.disabledButton]} disabled={saving} onPress={onDelete}>
          <Text style={styles.dangerButtonText}>Supprimer ce repas</Text>
        </Pressable>
      ) : null}
      ...
      {canReviewItems && meal.validation_blocked ? (
        <Text style={styles.warningText}>Corrige ou retire les aliments sans source fiable avant validation.</Text>
      ) : null}
      ...
      {canReviewItems ? (
        <View style={styles.reviewActions}>
          ...
          <Pressable style={[styles.secondaryButton, saving && styles.disabledButton]} disabled={saving || edits.length === 0} onPress={onApply}>
            <Text style={styles.secondaryButtonText}>Recalculer</Text>
          </Pressable>
          <Pressable
            ...
            onPress={onValidate}
          >
            <Text style={styles.primaryButtonText}>Valider dans ALIS</Text>
          </Pressable>
          {canDelete ? (
            <Pressable style={[styles.dangerButton, saving && styles.disabledButton]} disabled={saving} onPress={onDelete}>
              <Text style={styles.dangerButtonText}>Supprimer ce repas</Text>
...
function AnalysisTrace({ meal }: { meal: NutritionMeal }) {
  const datasetVersions = formatDatasetVersions(meal.dataset_versions);
  ...
  return (
    <View style={styles.tracePanel}>
      {meal.model_name ? <Text style={styles.traceText}>Modèle · {meal.model_name}</Text> : null}
      {meal.prompt_version ? <Text style={styles.traceText}>Prompt · {meal.prompt_version}</Text> : null}
      {datasetVersions ? <Text style={styles.traceText}>Sources · {datasetVersions}</Text> : null}
      {barcodes.length ? <Text style={styles.traceText}>Code-barres vu · {barcodes.join(', ')}</Text> : null}
    </View>
  );
}

function AnalysisJobPanel({ meal }: { meal: NutritionMeal }) {
  const job = meal.analysis_job;
  if (!job) {
    return null;
  }
  return (
    <View style={styles.jobPanel}>
      <Text style={styles.jobText}>{analysisJobText(meal)}</Text>
      {job.error_message ? <Text style={styles.jobErrorText}>{job.error_message}</Text> : null}
    </View>
  );
}

function ReviewDiagnosticPanel({ meal }: { meal: NutritionMeal }) {
  const rows = analysisStageRows(meal);
  return (
    <View style={styles.reviewGuidePanel}>
      <Text style={styles.sectionTitle}>Revue guidée</Text>
      <Text style={styles.stageText}>{confidenceInsight(meal)}</Text>
      {rows.map((row) => (
      ...
      <View style={styles.reviewChecklist}>
        <Text style={styles.checklistTitle}>Checklist revue</Text>
        <Text style={styles.secondaryText}>1. Sources nutritionnelles</Text>
        <Text style={styles.secondaryText}>2. Portions</Text>
    ...
    <View style={[styles.itemRow, missingSource && styles.itemRowNeedsSource, !included && styles.itemRowMuted]}>
      <View style={styles.itemHeader}>
        ...
        <View style={styles.itemHeaderActions}>
          {missingSource ? <Text style={styles.itemNeedsSourceBadge}>À corriger</Text> : null}
          <Pressable onPress={() => onIncludedChange(item.id, !included)}>
            <Text style={styles.itemToggle}>{included ? 'Inclus' : 'Retiré'}</Text>
          </Pressable>
        </View>
      </View>
      ...
      {missingSource ? <Text style={styles.itemGuidanceText}>Associe une source nutritionnelle ou retire cet aliment.</Text> : null}
      {missingSource ? (
        <Pressable style={styles.itemRemoveButton} onPress={() => onIncludedChange(item.id, false)}>
          <Text style={styles.itemRemoveButtonText}>Retirer cet aliment</Text>
        </Pressable>
      ) : null}
      {learnedReferences.length > 0 ? (
        <View style={styles.learnedPanel}>
          <Text style={styles.learnedTitle}>Propositions rapides</Text>
          {learnedReferences.map((reference) => (
            <Pressable key={reference.id} style={styles.learnedResult} onPress={() => onMatchFoodReference(item.id, reference)}>
              <View style={styles.matchResultHeader}>
                ...
                <Text style={styles.bestMatchBadge}>Appris</Text>
              </View>
              <Text style={styles.secondaryText}>
                {sourceLabel(reference.source)} · {Math.round(reference.energy_kcal_100g)} kcal/100g
              </Text>
              <Pressable style={styles.chooseMatchButton} onPress={() => onMatchFoodReference(item.id, reference)}>
                <Text style={styles.chooseMatchButtonText}>Réutiliser</Text>
              </Pressable>
            </Pressable>
          ))}
        </View>
      ...
      <View style={styles.matchPanel}>
        <View style={styles.matchInputRow}>
          <TextInput
            ...
            placeholder="Chercher CIQUAL ou Open Food Facts"
            autoCapitalize="none"
          />
          <Pressable
          ...
          >
            <Text style={styles.compactButtonText}>Chercher</Text>
          </Pressable>
        </View>
        {searchCompleted && searchResults.length === 0 ? (
          <View style={styles.emptyMatchPanel}>
            <Text style={styles.emptyMatchTitle}>Aucune source trouvée</Text>
            <Text style={styles.secondaryText}>Essaie un nom plus générique ou retire cet aliment.</Text>
          </View>
        ) : null}
        {searchResults.map((reference, index) => (
          <Pressable key={reference.id} style={styles.matchResult} onPress={() => onMatchFoodReference(item.id, reference)}>
            <View style={styles.matchResultHeader}>
              <Text style={styles.matchResultTitle}>{reference.name}</Text>
              {index === 0 ? <Text style={styles.bestMatchBadge}>Meilleur match</Text> : null}
            </View>
            <Text style={styles.secondaryText}>
              {sourceLabel(reference.source)} · {Math.round(reference.energy_kcal_100g)} kcal/100g
            </Text>
            <Pressable style={styles.chooseMatchButton} onPress={() => onMatchFoodReference(item.id, reference)}>
              <Text style={styles.chooseMatchButtonText}>Choisir</Text>
            </Pressable>
          </Pressable>
        ))}
      </View>
